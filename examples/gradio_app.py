#!/usr/bin/env python3
"""
Copyright (c) 2024 Boson AI. All rights reserved.
...
def get_voice_path(voice_name: str) -> str | None:
...
def generate_speech(
    text: str, voice: str, temperature: float, top_p: float, seed: int, mode: str = "full"
) -> tuple[str, str]:
    """
            ...
            with gr.Column(scale=2):
                ...
                with gr.Row():
                    voice_dropdown = gr.Dropdown(
                        choices=voices, value=default_voice, label="Voice", info="Select a voice for generation"
                    )

                    seed_input = gr.Number(value=42, label="Seed", info="Random seed for reproducible results")
...
def main():
    ...
    interface.launch(server_name=args.host, server_port=args.port, share=False, show_error=True)
